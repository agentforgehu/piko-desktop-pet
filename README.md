...
- 支持 OpenAI API 与回环地址上的 OpenAI-compatible 本地模型，模型文字、情绪和动作会进入 PetMind；
- 可配置称呼、性格和低/中/高主动程度；模型不可用时自动回退到本地表达；
- 设置可保存并测试模型连接；托盘实时显示关闭、未测试、已连接或具体故障；
- 深青绿圆形大写 `P` 托盘标识；
- 独立 World Lab，用于查看窗口几何和可站立表面。

默认不联网，不读取文件内容，不记录窗口标题。截图、浏览器内容、完整代码和键盘内容始终不在默认授权范围内。开发感知、Git、记忆、模型和 Agent 读取权限均默认关闭。
...
`0.2.4` 是未签名的 Pre-release，可使用 `scripts/publish.ps1` 生成单文件当前用户安装器、自包含便携包、更新清单和 VS Code 扩展。版本由 `release-version.txt` 单点驱动；文档、安装器和扩展不同步时发布会失败。正式 `1.0` 的发布脚本默认拒绝生成未签名稳定版。

常用操作：

| 操作 | 结果 |
|---|---|
| 单击 Piko | 打开互动窗口，回复同步到宠物表情与动作 |
| 拖动 Piko | 抱起并放到窗口或桌面上 |
| 双击 Piko | 打开设置 |

...
当前开发目标：1.0.0 正式版候选（尚未正式发布）

最新状态与证据见 [1.0 发布准备](RELEASE_1.0_READINESS.md)。本表后文为历史基线，不可据此跳过正式验收。
...
| AI Provider | OpenAI Responses API 与回环 OpenAI-compatible 本地模型、本地凭据、结构化输出、`store=false`、连接测试、托盘健康状态、禁用/无 Key 零网络 | DONE（真实账号调用待发布者验收） |
...
- Setup 安全测试：6/6；
- Update 安全测试：9/9；
- 总自动化测试：以 `scripts/verify.ps1` 当前输出为准，发布不得手改历史数字；
